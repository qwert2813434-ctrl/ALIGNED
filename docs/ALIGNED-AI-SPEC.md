...
anim      選填 出場動畫（見〈出場動畫〉）；未設＝靜態，舊版 App 打開零影響
...
每個 block 可以掛一個出場動畫；有任何一塊掛了，App 的匯出就會走影片。
不掛＝靜態版面。整包欄位長這樣（全部選填，只有 `kind` 必填）：

```
anim: {
  kind    文字四種："typewriter"（逐字）/"textPhrase"（逐句）/"textSlide"（位移）/"textFlicker"（隨機閃現）
          物件四種："slide"（位移）/"fade"（淡入）/"scale"（縮放）/"maskWipe"（遮罩擦入）
          文字給文字組、圖片影片色塊給物件組——App 面板就是這樣分的，掛錯組
          使用者在面板上看不到對應選項、一調就被換掉
  dir     "up"/"down"/"left"/"right"，只有物件吃（預設 left＝從左進來）；
          文字的方向是固定的（開頭→尾端），設了會被忽略
  dur     入場秒數。**不給最好**——逐字/逐句/閃現的預設會跟字數走（約 8 字/秒），
          自己給一個 0.9 秒的值，長文就一閃而過
  delay   起始延遲秒數，預設 0。編排「先標題、後內文、再圖」就是靠每塊給遞增的 delay
  stage2  "scale"/"fullscreen"  兩段式：入場後接著放大到第二個位置並停住
          （scale＝放大到 scale2 倍、預設 1.5；fullscreen＝蓋滿整頁）。只有物件吃
  dur2 / scale2   第二段秒數（預設 0.6）與倍率
}
```

Project 層還有兩個對應欄位：`animHold`＝跑完停留秒數（預設 5，IG 縮圖取的是
**最後一幀**，所以停留時的版面就是封面）；`animStagger`＝「陸續出現」的間隔秒數（預設 0.2）。

編排的手感：同頁 2–4 塊掛動畫就夠，全部掛會很吵；delay 依閱讀順序遞增
（0 / 0.3 / 0.6…），標題用文字效果、圖用 fade 或 slide 是不會錯的組合。
...
8. 逐字動畫自己給 `dur` → 長文一閃而過；不給，預設會跟字數走
9. 匯入/開啟後 App 會換新 UUID，同一檔開兩次不會互相覆蓋——放心重複給檔
